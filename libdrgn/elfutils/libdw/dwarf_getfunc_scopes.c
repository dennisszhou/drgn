...
pc_match (unsigned int depth, struct Dwarf_Die_Chain *die, void *arg)
{
  ...
  if (a->scopes != NULL)
      ...
      if (result < 0)
	{
      ...
      if (depth && INTUSE (dwarf_tag) (&die->die) == DW_TAG_inlined_subroutine)
	      return pc_record(depth, die->parent, arg);
    }
...
int
dwarf_getfunc_scopes (Dwarf_Die *cudie, Dwarf_Addr pc, Dwarf_Die **scopes)
{
  if (cudie == NULL)
    return -1;
